...
public class Time extends Thread {
	...
	public void run() {
		synchronized (main){
			{
				System.out.println(Thread.currentThread().getName()+"创建抢地主、出牌线程");
				while (i > -1 && isRun) {
					main.time[1].setText("倒计时:" + i--);
					second(1);
				}
				if (i == -1)
					main.time[1].setText("不抢");//计时器结束默认为不抢
				main.landlord[0].setVisible(false);
				main.landlord[1].setVisible(false);
				for (Card card2 : main.playerList[1])
					card2.canClick = true;// 计时器结束后或点击按钮后可被点击
				if (main.time[1].getText().equals("抢地主")) {
					...
					main.publishCard[1].setEnabled(false);//抢完地主不能不出牌
					setlord(1);
				} else {
				...
				main.turn = main.dizhuFlag;//地主先出牌
				while (true) {
					if (main.turn == 1) {//当你出的牌time[0]time[2]都不要时
						if (main.time[0].getText().equals("不要") && main.time[2].getText().equals("不要"))
	...
	public void openlord(boolean is) {//打开地主牌
		for (int i = 0; i < 3; i++) {
			if (is)
	...
	public void setlord(int i) {//设置地主的头像
	...
	public void ShowCard(int role) {
		...
		if (main.time[(role + 1) % 3].getText().equals("不要") && main.time[(role + 2) % 3].getText().equals("不要")) {
			...
			if (role != main.dizhuFlag) {
				...
				if ((role + 1) % 3 == main.dizhuFlag) {
					if ((Common.jugdeType(main.currentList[(role + 2) % 3]) != CardType.c1
						...
						f = 1;
					if (main.currentList[(role + 2) % 3].size() > 0
							&& Common.getValue(main.currentList[(role + 2) % 3].get(0)) > 13)
						f = 1;
				}
				if (f == 1) {
			...
			int can = 0;
			if (role == main.dizhuFlag) {
				if (main.playerList[(role + 1) % 3].size() <= 5 || main.playerList[(role + 2) % 3].size() <= 5)
					can = 1;
			}
			else {
			...
			if (cType == CardType.c1) {
				if (can == 1)
					model = Common.getModel(main.playerList[role], new int[] { 1, 4, 3, 2, 5 });
				AI_1(model.a1, player, list, role);
			}
			else if (cType == CardType.c2) {
				if (can == 1)
					model = Common.getModel(main.playerList[role], new int[] { 2, 4, 3, 5, 1 });
				...
				if (can == 1)
					model = Common.getModel(main.playerList[role], new int[] { 3, 1, 4, 2, 5 });
				AI_2(model.a3, model.a1, player, list, role);
			}
			else if (cType == CardType.c32) {
				if (can == 1)
					model = Common.getModel(main.playerList[role], new int[] { 3, 2, 4, 5, 1 });
				...
				if (can == 1)
					model = Common.getModel(main.playerList[role], new int[] { 5, 3, 2, 4, 1 });
				AI_3(model.a123, player, list, role);
			}

			else if (cType == CardType.c112233) {
				if (can == 1)
					model = Common.getModel(main.playerList[role], new int[] { 2, 4, 3, 5, 1 });
	...
	public boolean win() {
		for (int i = 0; i < 3; i++) {
			if (main.playerList[i].size() == 0) {
				...
				System.out.println(Thread.currentThread().getName()+"抢地主、出牌线程结束");
				return true;
			}
		}
		return false;
